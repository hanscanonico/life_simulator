...
- 2026-09-24 — **Where the programme stands after sweeps 9 and 10: every pre-registered
  sweep has run, five substrate bets are spent, and the next substrate is an open
  ...
  **The five bets on rung 4** ("does complexity keep rising"), each read by the finding that
  ...
  | Contested energy | 9, `host-parasite` | every priced arm at or below the control at its cap; the four 512-influx arms barren | 1 of 12 priced arms **keeps rising**, on 1 of 2 measured runs; 2 plateau, 3 neither, 2 unread; both controls **neither**; the pre-registered rule reads 25 of 26 emerged runs unmeasured and no arm | theft evolved in all six steal arms (peak `steal_rate` 0.32–0.94) | `complexity-under-contest` (`partial`) |
  ...
  **What the corpus says about "does complexity keep rising".** Across every substrate
  ...
  rising run of 5 and 1 of 4. The one priced arm that clears the pre-registered bar does so
  at the smallest n the rule reads, beside a control that holds a rising run too, and pooled
  over runs the priced arms rise no more often than the controls (3 of 17 against 2 of 9,
  descriptive). Every one of the five bets lowered the emergence rate against its own
  control, and three treatments abolished it outright (the `2048` cost, the 512 influx, the
  host mode); since the complexity reading is taken on emerged runs only, each arm's reading
  rests on 0 to 6 runs, and in sweep 9 the post-crossing span those runs are read over has a
  median of 9 460 epochs (80 to 16 660) inside a 20 000-epoch budget. The mechanism the
  2026-09-16 investigation found — a drift–selection balance in which a byte off the copy
  path costs nothing — is not contradicted by anything sweeps 9 and 10 read.
  ...
  **The same 180 worlds, read three times.** Sweep 8's cap-128 and cap-256 arms, sweep 9's
  ...
  two independent ones, and about 360 runs of compute went to reproducing worlds the lab
  already held.
  ...
  **The literature #191 weighed, against what sweeps 9 and 10 actually showed.** #191 took
  ...
  Sweep 9 reproduced the first half of that here — theft evolved wherever it was offered —
  and not the second: complexity did not rise with it. Sweep 10's host mode is the
  Tierra-shaped asymmetry, and it never produced a replicator at all; Tierra's parasites
  evolve from a hand-written ancestor, so that literature never asked the host–parasite
  mechanism to also produce the first replicator, which is what this programme's design
  couples it to. The BFF group's 2026 paper (arXiv 2607.01483), which predicts a plateau for
  the pairing dynamic, stands; the task-modulated pairing result (arXiv 2607.09211) remains
  the only one cited with a rising capability ladder, and it imports an objective.
  ...
  **The open decision.** Which substrate, or which question, the next sweep takes. The
  ...
  1. **Decide the one rising arm.** More seeds on `1024×2048 128` and its control `0 128`.
     *For*: it is the only arm in the corpus that clears the pre-registered bar, and the
     machinery exists. *Against*: it stands on 1 rising run of 2; the control rises at a
     similar per-run rate; at the rates the two arms showed (2 and 5 of 90), 180 more seeds
     each add roughly four emerged runs to the priced arm and ten to the control. About half
     a day of the mini-pc for 360 runs.
  ...
  3. **Start the treatments from an emerged world.** Take the post-crossing snapshots of the
     ...
     every bet lowered emergence and host mode abolished it, so sweeps 9 and 10 read the
     complexity question on 0 to 5 runs an arm and asymmetry is unreadable rather than
     refuted; the host–parasite literature starts from an ancestor. *Against*: it is a new
  ...
  5. **A richer instruction set.** *For*: the energy-and-theft result is on a Z80 soup and
     sweep 9 reproduced only its first half on BFF, so what a tape can express may be the
     binding difference. *Against*: a new substrate's engineering cost, and no measurement
     here isolates the instruction set as the constraint — sweep 5 only removed ops.
  6. **Close rung 4 on Soup and spend compute below it.** The evolution programme entry
     ...
     and the plateau's mechanism is explained, while the rungs below it have never had their
     own designed sweep. *Against*: it leaves the programme's headline question answered
     only in the negative, and only for this substrate.

  Independently of which is chosen: the pending relock of `transition_epoch` (#229, #237)
  moves no reading above — emergence here is census- or copy-confirmed, and the 2026-09-19
  rescore left every arm at cap ≤ 256 unchanged — and any future sweep whose control is the
  default substrate at caps 128/256 on seeds 1–90 can read the 180 stored worlds rather than
  run them a fourth time.
